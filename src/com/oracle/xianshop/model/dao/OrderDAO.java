...
public interface OrderDAO {
	 ...
	 * 从购物车表中删除订单详情表的dao方法
	 */
	@Delete("delete from items where orderid=#{orderid} and goodsid=#{goodsid}")
	public int deleteOrderItemFromItems(@Param("orderid")String orderid,@Param("goodsid")int goodsid);
	
	@Select("select count(*) from  Items where orderid=#{orderid}")
	public int selectCountGoodsInOdrder(@Param("orderid")String orderid);
}
